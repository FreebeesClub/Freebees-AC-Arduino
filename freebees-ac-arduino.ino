...
void DeviceTimeToSerial() {
  ...
  sprintf(devtime, "%02d-%02d-%02dT%02d:%02d:%02d.000Z", now.year(), now.month(), now.day() % 10, now.hour(), now.minute(), now.second());
...
void sendSyncStatus(bool syncStatus) {
  DynamicJsonBuffer jsonBuffer2;
  JsonObject& json2 = jsonBuffer2.createObject();
  json2["status"] = syncStatus;
  json2["devid"] = devId;
  char buffer[json2.measureLength()];
  json2.printTo(buffer, sizeof(buffer) + 1);
...
void handleSync(byte * payload) {
  ...
  if (json.success()) {
    ...
    int accessLimit = 1;
    //json.prettyPrintTo(Serial);
    if (json["action"] == "sync" && json["total"]) {
      accessTotal = json["total"];
      String jsonStr = "{\"action\":\"get\",\"start\":0,\"limit\":" + String(accessLimit) + "}";
      client_MQTT.publish((topic_part + "/ac/sync").c_str(), jsonStr.c_str());
    } else if (accessCount <= accessTotal - 1) {
      ...
      String jsonStr = "{\"action\":\"get\",\"start\":" + String(accessCount) + ", \"limit\":" + String(accessLimit) + "}";
      client_MQTT.publish((topic_part + "/ac/sync").c_str(), jsonStr.c_str());
    } else {
      sendSyncStatus(true);
    }
